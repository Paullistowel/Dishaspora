...
export default function Subscription() {
  ...
  const subscribe = useMutation({
    ...
    onError: () => toast.error(t('orders.paymentStartError')),
  });
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.background, paddingTop: insets.top + 6 }}>
      <ScreenHeader title={t('orders.premiumTitle')} />
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 60, gap: 18 }}>
        {user?.premium ? (
          <Animated.View entering={FadeInDown.duration(300)} style={styles.activeCard}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={styles.activeTitle}>{t('orders.premiumActive')}</Text>
              <Text style={styles.activeSub}>
                {user.premiumUntil
                  ? `${t('orders.renewsExpires')} ${new Date(user.premiumUntil).toDateString()}`
                  : t('orders.premiumEnjoy')}
              </Text>
            </View>
          </Animated.View>
        ) : null}
        ...
        <Animated.View entering={FadeInDown.delay(60).duration(300)} style={styles.freeCard}>
          <Text style={styles.freeTitle}>{t('orders.freePlan')}</Text>
        ...
        {plans.isLoading ? (
          ...
          <View style={styles.plansError}>
            <Text style={styles.plansErrorText}>{t('orders.plansError')}</Text>
            <PrimaryButton
          ...
          (plans.data ?? []).map((plan, i) => (
            ...
            >
              ...
              <PrimaryButton
                title={user?.premium ? t('orders.extendSubscription') : t('orders.subscribe')}
